...
public class SelectAll {
            ...
            if(connection !=null){
                if(id == 1){
                    ...
                    sql = "select DISTINCT KHACHSAN.MaKS , TenKS ,Hang , ChinhSachVS , DiaDiem ,Longitude ,Latitude,TimeDat,TimeTra ,SoLuongP ,MoTa ,LOAIHINH.TenLH,GiaDD,\n" +
                            ...
                            "join CHUKHACHSAN on CHUKHACHSAN.MaKS = KHACHSAN.MaKS\n" +
                            "join CHECKTT on CHECKTT.MaKS = KHACHSAN.MaKS\n" +
                            "where LOAIHINH.TenLh like N'Chung cư' ";
                }else if(id == 5){
                    sql = "select DISTINCT KHACHSAN.MaKS , TenKS ,Hang , ChinhSachVS , DiaDiem ,Longitude ,Latitude,TimeDat,TimeTra ,SoLuongP ,MoTa ,LOAIHINH.TenLH,GiaDD,\n" +
                            "TIENNGHIHD.WifiSanh,WifiPhong,BeBoi,DauXe,Spa,VatNuoi,DieuHoa,NhaHang,Bar,Gym ,\n" +
                            " CHUKHACHSAN.SDT , CHUKHACHSAN.Ten\n" +
                            ...
                            "join CHUKHACHSAN on CHUKHACHSAN.MaKS = KHACHSAN.MaKS\n" +
                            "join CHECKTT on CHECKTT.MaKS = KHACHSAN.MaKS\n" +
                            "where LOAIHINH.TenLh like N'Homestay' ";
                }
                Statement st = connection.createStatement();
                ResultSet rs = st.executeQuery(sql);
                while(rs.next()) {
    ...
    public void updateInFoUser(String userName, String toString, String toString1, String toString2, String toString3, byte[] imageViewToByte) {
        try {
            String image = Base64.encodeToString(imageViewToByte , Base64.DEFAULT);
            String sql = "update KHACHHANG set KHACHHANG.CMND = '"+toString+"',  KHACHHANG.Email = '"+toString1+"', KHACHHANG.DiaChi = N'"+toString2+"' , KHACHHANG.SDT = '"+toString3+"' ,KHACHHANG.Anh = '"+image+"'  where KHACHHANG.UserName like '"+userName+"' ";
            Statement st = connection.createStatement();
            st.executeUpdate(sql);
    ...
    public boolean checkemail(String u){
        try {
            String sql = "select * from KHACHHANG where Email = '"+u+"'";
            Statement st = connection.createStatement();
            ResultSet rs = st.executeQuery(sql);
            if(rs.next()){
                return true;
    ...
    public void updatePassword(String userName, String toString) {
        try {
            String sql = "update KHACHHANG set KHACHHANG.Pass = '"+toString+"' where KHACHHANG.Email like  N'"+userName+"'";
            Statement st = connection.createStatement();
            st.executeUpdate(sql);
